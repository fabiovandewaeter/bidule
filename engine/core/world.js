...
import * as GameEventSchedule from './event/event_schedule.js'
import * as Event from './event/event.js'

/**
 * @typedef {Object} World
 * @property {Clock} clock
 * @property {GameEventSchedule} events
 */
...
export function create() {
    return {
        ...
        events: GameEventSchedule.create(),
    };
}
 ...
 *
 * @param {World} world
 * @param {number} [target_time] - défaut: maintenant
 */
export function advance_to(world, target_time = Date.now()) {
    /** @type {Schedule} */
    const schedule = (type, at, payload) => GameEventSchedule.schedule(world.events, type, at, payload);

    while (true) {
        const event = GameEventSchedule.pop_due(world.events, target_time);
        if (!event) break;
...
export function next_event_at(world) {
    const next = GameEventSchedule.peek(world.events);
